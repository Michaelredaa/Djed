...
def get_dict(base_list, key):
    my_dict = [x for x in base_list if x.get('name') == key]
    if not my_dict:
        return False
    else:
        return my_dict[0]
...
if __name__ == '__main__':
    ...
    print(get_shading_node('maya', 'arnold'))
    shading_nodes_conversion('clarisse', 'autodesk_standard_surface', 'maya', 'arnold')
